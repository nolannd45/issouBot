...
@bot.command()
async def heydar(ctx):
    guild = ctx.guild
    voice_client: discord.VoiceClient = discord.utils.get(bot.voice_clients, guild=guild)
    audio_source = discord.FFmpegPCMAudio("/home/orange/test/issoubot/mp3/est_ce_que.mp3")
...
async def badissou(ctx):
    guild = ctx.guild
    voice_client: discord.VoiceClient = discord.utils.get(bot.voice_clients, guild=guild)
    audio_source = discord.FFmpegPCMAudio('/home/orange/test/issoubot/mp3/badissou.mp3')
    if not voice_client.is_playing():
        voice_client.play(audio_source, after=None)
...
async def pf(ctx):
    ...
    if piece == 1:
        await ctx.send("""
        ----------------------------------------
        La pièce est tombée sur face, cheeeeh !!
        ----------------------------------------
        """)
    else:
        await ctx.send("""
        -------------------------------------------------
        La pièce est tombée sur pile , bien joué negro !!
        -------------------------------------------------
        """)
...
@bot.command()
async def infoISSOU(ctx):
  await ctx.send("""
**Créateurs du bot :** Le désanusseur (Nolann), Superéjaculator (Zakaria), BRANLIX 2000 (Mathias) !

**-helpISSOU :** Liste des commandes disponibles actuellement.

 *Si vous avez des idées d'implémentations/d'innovations liées au IssouBOT, n'hésitez pas à DM les créateurs du bot*

ENJOY ! <:issou:780744685718601738>
""") 

@bot.command()
async def helpISSOU(ctx):
  await ctx.send("""
---------------------------
commande de kick: 
    -kick @usurname
  commande pour les devoirs: 
    -devoir (permet de voir la liste des devoirs)
    -addDevoir <nom du devoir> <DD MM AAAA>
    -suppDevoir <nom du devoir>
---------------------------

 <:issou:780744685718601738>
""") 
